...
def load_text_hierarchical(text, max_sentence_nb, maxlen, input2idx,stopwords):
	text = list(jieba.cut(text))
	text = remove_stopwords(stopwords,text)
...
def load_X_hierarchical(file_path, max_sentence_nb, maxlen, input2idx, stopwords):
	...
	with open(file_path) as f:
		for i in tqdm(f):
			...
			vectors = load_text_hierarchical(i, max_sentence_nb, maxlen, input2idx,stopwords)
			ret.append(vectors)
	ret = np.asarray(ret, dtype='int32')
	return ret
...
def train_vector(input_set,vector_size, min_count, words_path, bin_path):
	...
	for i in input_set:
		...
		f.write(' ')
	f.close()
	word2vec.word2vec(words_path, bin_path,cbow=1, size=vector_size,min_count=1, verbose=True)
