...
from util_hi import plot_loss_figure, load_X, load_Y,load_X_hierarchical, n_hot_decoder, unique, get_stopwords
from util_hi import get_input_set, list2str, get_label_set, flatten, concat
from util_hi import save_input2idx, save_label2idx, load_input2idx, load_label2idx,train_vector,load_matrix
...
from hierarchical import build_hierarchical
from local2 import build_local
from sent_cnn_local import build_local
...
def main():
	...
	save_dir = '../30w_weights/att_' + str(datetime.now()).split('.')[0].split()[0] + '/' # model is saved corresponding to the datetime
	...
	choice = "char"
	stopwords_path = '../train_data/law_stopwords_v2.txt'
	words_path = '../train_data/save/words_30w.txt'
	bin_path = '../train_data/save/vector_30w.bin'
	#weights_file_path = '../model_30w/gru94_2017-01-05/weights.37-0.044.h5'
	weights_file_path = '../model_30w/hi_2017-01-12/weights.17-0.045.h5'
	
	#stopwords = get_stopwords(stopwords_path)
	stopwords = ['本院认为','本院','了','的','，']
	print(len(stopwords))

	
	input_set, input2idx = get_input_set(label_data_path,min_count,stopwords)
	...
	#model = build_local(input_set_size, vector_size, maxlen,max_sentence_nb, label_set_size)
	#model = build_hierarchical(input_set_size, vector_size, maxlen,max_sentence_nb, label_set_size,matrix)
	#model = build_cnn_bgru_attention(input_set_size, vector_size, max_sentence_len, label_set_size)	
	#model = build_blstm(input_set_size, vector_size, max_sentence_len, label_set_size)
	#model = build_cnn_blstm_vec(input_set_size, vector_size, max_sentence_len, label_set_size, matrix)

	#train_x = load_X(train_data_path, max_sentence_len, input2idx,stopwords)
	#val_x = load_X(val_data_path, max_sentence_len, input2idx,stopwords)
	...
	#model = load_model(weights_file_path)
	#test_x = load_X(test_data_path, max_sentence_len, input2idx, choice)
