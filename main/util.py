...
def load_text(text, max_sentence_len, input2idx, choice):
	if choice == 'char':
	...
	text = list(jieba.cut(text))
	stop = ['本院认为','了','的']
	text = [i for i in text if stop not in i]
       '''
...
def load_X(file_path, max_sentence_len, input2idx, choice):
	print('Loading x...')
	ret = []
	with open(file_path) as f:
		for i in tqdm(f):
			i = json.loads(i)['input_text']
			vectors = load_text(i, max_sentence_len, input2idx,choice)
			ret.append(vectors)
