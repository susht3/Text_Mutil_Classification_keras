...
from util94 import plot_loss_figure, load_X, load_Y,load_X_hierarchical, n_hot_decoder, unique, get_stopwords
from util94 import get_input_set, list2str, get_label_set, flatten, concat
from util94 import save_input2idx, save_label2idx, load_input2idx, load_label2idx,train_vector,load_matrix
...
def main():
	...
	save_dir = '../model_30w/gru512_' + str(datetime.now()).split('.')[0].split()[0] + '/' # model is saved corresponding to the datetime
	...
	#model = build_hierarchical(input_set_size, vector_size, maxlen,max_sentence_nb, label_set_size)
	...
	model = build_cnn_bgru_vec(input_set_size, vector_size, max_sentence_len, label_set_size, matrix)
