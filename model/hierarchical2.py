...
def build_sentence_model(input_set_size,vector_size,maxlen,matrix):
	...
	embed1 = Embedding(input_set_size, vector_size, input_length=maxlen)(word_inputs)
	embed2 = Embedding(input_set_size, vector_size,
				weights=[matrix],
				input_length=maxlen,
				trainable=False)(word_inputs)
	embed = merge([embed1,embed2],mode='sum')
	...
	bn1 = BatchNormalization(mode=1)(embed)
	drop1 = Dropout(0.2)(bn1)
        #gru
	bgru = Bidirectional(GRU(192,return_sequences=False), merge_mode='sum')(drop1)
