...
def log_normalization(vector, maxn):
	log_vector = []
	for i in vector:
		x = math.log(1+i)/math.log(1+maxn) 
		log_vector.append(x)
	return log_vector

def load_text_id_vector(text, max_sentence_len, input2idx, choice):
	if choice == 'char':
...
def train_tf_idf(id_vector):
	transformer = TfidfTransformer()
	tfidf = transformer.fit_transform(counts)
	vectors = tfidf.toarray()
	return vectors

def load_X_td_idf(file_path, max_sentence_len, input2idx, choice):
	print('Loading x...')
	ret = []
	with open(file_path) as f:
		for i in tqdm(f):
			i = json.loads(i)['input_text']
			vectors = load_text_id_vector(i, max_sentence_len, input2idx,choice)
			ret.append(vectors)
	ret = train_td_idf(ret)
	#ret = np.asarray(ret, dtype='int32')
	return ret
    
def load_text_bag_of_words(text, max_sentence_len, input_set, choice):
	if choice == 'char':
		#text = remove_stopwords(text)
		text = ''.join(filter(lambda x: x in input_set, list(text)))
	elif choice == 'word':
		text = list(jieba.cut(text))
		text = [ i for i in text if i  in input_set ]

	maxn = 0
	vectors = []
	input_set_size = len(input_set)
	for j in range(input_set_size):
		char = input_set[j]
		n = text.count(char)
		vectors.append(n)
		if n > maxn:
	...
	vectors = log_normalization(vectors, maxn)
	vectors = np.asarray(vectors, dtype='int32')
	return vectors


def load_X_bag_of_words(file_path, max_sentence_len, input_set, choice):
	print('Loading x...')
	ret = []
	with open(file_path) as f:
		for i in tqdm(f):
			i = json.loads(i)['input_text']
			vectors = load_text_bag_of_words(i, max_sentence_len, input_set,choice)
			ret.append(vectors)
	ret = np.asarray(ret, dtype='int32')
	return ret
...
def get_input_set(file_path, choice,min_count):
